...
-- | A guessing game
module Plutus.Contracts.LottoV2
    ( lock
    , guess
    , game
    , GameSchema
...
data Game
instance V2.ValidatorTypes Game where
    type instance RedeemerType Game = ClearString
    type instance DatumType Game = HashedString

gameInstance :: V2.Validator Game
gameInstance = V2.mkTypedValidator @Game
    $$(PlutusTx.compile [|| validateGuess ||])
    $$(PlutusTx.compile [|| wrap ||]) where
        wrap = V2.mkUntypedValidator @HashedString @ClearString
...
validateGuess :: HashedString -> ClearString -> V2.ScriptContext -> Bool
...
gameValidator = V2.validatorScript gameInstance
